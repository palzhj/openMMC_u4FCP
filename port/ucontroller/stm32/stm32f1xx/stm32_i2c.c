 ...
 *
 * @retval 0           Operation succeeded (data was successfully transmitted and received).
 * @retval 1           Timeout occurred while waiting for the I2C to be idle.
 * @retval 2           Timeout occurred while waiting for the START condition to be transmitted.
 * @retval 3           Timeout occurred while waiting for the slave address acknowledgment.
 * @retval 4           Timeout occurred while waiting for the data byte to be transmitted.
 * @retval 5           Timeout occurred while waiting for the data byte to be received.
 * @retval 6           Timeout occurred while waiting for the STOP condition.
 */
int xI2CMasterWriteRead(I2C_ID_T id, uint8_t addr, const uint8_t *tx_buff, uint8_t tx_len, uint8_t *rx_buff, uint8_t rx_len)
{
  ...
  while (!I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) {
      if (timeout-- == 0) return 3;  // Timeout error
  }
  ...
  for (i = 0; i < tx_len; i++) {
      ...
      while (!I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) {
          if (timeout-- == 0) return 4;  // Timeout error
      }
  }
  ...
  // Wait for the START condition to be transmitted again
  timeout = I2C_TIMEOUT;
  while (!I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_MODE_SELECT)) {
      if (timeout-- == 0) return 2;  // Timeout error
  }
  ...
  // Read data byte-by-byte from the slave
  for (i = 0; i < rx_len; i++) {
      ...
      while (!I2C_GetFlagStatus(I2Cx, I2C_FLAG_RXNE)) {
          if (timeout-- == 0) return 5;  // Timeout error
      }
  ...
  while (I2C_GetFlagStatus(I2Cx, I2C_FLAG_STOPF)) {
      if (timeout-- == 0) return 6;  // Timeout error
  }
 ...
 *
 * @retval 0           Operation succeeded (data was successfully transmitted).
 * @retval 1           Timeout occurred while waiting for the I2C to be idle.
 * @retval 2           Timeout occurred while waiting for the START condition to be transmitted.
 * @retval 3           Timeout occurred while waiting for the slave address acknowledgment.
 * @retval 4           Timeout occurred while waiting for data byte transmission.
 *
 * @note   This function assumes that the I2C peripheral has already been initialized and
 *         configured before calling this function. The I2C timeout is defined by the
 *         `I2C_TIMEOUT` macro, which can be adjusted as needed for different use cases.
 *         The function does not handle any retries for transmission errors or NACK responses
 ...
 *
 * @retval 0           Operation succeeded (data was successfully received).
 * @retval 1           Timeout occurred while waiting for the I2C to be idle.
 * @retval 2           Timeout occurred while waiting for the START condition to be transmitted.
 * @retval 3           Timeout occurred while waiting for the slave address acknowledgment.
 * @retval 5           Timeout occurred while waiting for data byte reception.
 * @retval 6           Timeout occurred while waiting for the STOP condition.
 */
